...
export function VerticalTabsEditor({ data, onChange }: { data: BlockData<VerticalTabsBlockData>; onChange: (d: BlockData<VerticalTabsBlockData>) => void }) {
          ...
          <div className="space-y-4">
            ...
            {(data.tabs || []).map((item: any, idx: number) => (
              <div key={idx} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-4 relative">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Nhãn (Menu)"><Inp value={item.label || ""} onChange={v => updateItem(idx, "label", v)} /></Field>
